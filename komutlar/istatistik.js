...
exports.run = (client, message, args) => {
    ...
    const wia = new Discord.MessageEmbed().setTitle('İstatistik').setThumbnail(client.user.displayAvatarURL({size: 1024})).addField('Kullanıcı Sayısı', toplam, true).addField('Sunucu Sayısı', client.guilds.cache.size, true).addField('Kanal Sayısı', client.channels.cache.size, true).addField('Çalışma Süresi', uptime, true).addField('Node.JS Versiyon', process.version, true).addField('Ram Kullanımı', (process.memoryUsage().heapUsed / 1024 / 1024).toFixed(0), true).addField('CPU', `\`\`\`${os.cpus().map(i => i.model)[0]}\`\`\``).addField(`Site adresi`, ` [Site Adresim](https://tidy-hilarious-asiago.glitch.me/)`, true).addField(`Destek Sunucu`, ` [Destek Sunucum](https://discord.gg/kHRASTQaFD) `, true).addField( true)
